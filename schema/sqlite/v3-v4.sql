...
/* Triggers for reacji_votes table */
...
CREATE TRIGGER increase_reacji_karma_count AFTER INSERT ON reacji_votes
  WHEN NEW.amount = 1 BEGIN
    ...
    INSERT OR IGNORE INTO karma_received_count
        VALUES (NULL, (
            SELECT message FROM reacji_message WHERE ID = NEW.reacji_message_id
        ), 0, 0, 0);
    INSERT OR IGNORE INTO karma_received_count
        VALUES (NULL, (
            SELECT cleaned_nick FROM nick_metadata
    ...
    UPDATE karma_received_count SET up = up + NEW.action  WHERE name = (
        SELECT message FROM reacji_message WHERE ID = NEW.reacji_message_id
    );
    UPDATE karma_received_count SET up = up + NEW.action
        WHERE name = (
            SELECT cleaned_nick FROM nick_metadata
                INNER JOIN reacji_message ON nick_metadata.id = reacji_message.nick_id
                WHERE reacji_message.id = NEW.reacji_message_id
...
CREATE TRIGGER decrease_reacji_karma_count AFTER INSERT ON reacji_votes
  WHEN NEW.amount = -1 BEGIN
    ...
    UPDATE karma_received_count SET down = down + NEW.action
        WHERE name = (
            SELECT message FROM reacji_message WHERE ID = NEW.reacji_message_id
        );
    UPDATE karma_received_count SET down = down + NEW.action
...
CREATE TRIGGER increase_sidevote_reacji_karma_count AFTER INSERT ON reacji_votes
  WHEN NEW.amount = 0 BEGIN
    ...
    UPDATE karma_received_count SET side = side + NEW.action
        WHERE name = (
            SELECT message FROM reacji_message WHERE ID = NEW.reacji_message_id
        );
    UPDATE karma_received_count SET side = side + NEW.action
...
CREATE TRIGGER delete_increase_reacji_karma_count_add AFTER DELETE ON reacji_votes
  WHEN OLD.amount = 1 BEGIN
    ...
    UPDATE OR IGNORE karma_received_count SET up = up - OLD.action
        WHERE name = (
            SELECT message FROM reacji_message where ID = OLD.reacji_message_id
        );
    UPDATE karma_received_count SET up = up - OLD.action
        WHERE name = (
            SELECT cleaned_nick FROM nick_metadata
                INNER JOIN reacji_message ON nick_metadata.id = reacji_message.nick_id
...
CREATE TRIGGER delete_decrease_reacji_karma_count_add AFTER DELETE ON reacji_votes
  WHEN OLD.amount = -1 BEGIN
    ...
    UPDATE OR IGNORE karma_received_count SET down = down - OLD.action
        WHERE name = (
            SELECT message FROM reacji_message where ID = OLD.reacji_message_id
        );
    UPDATE karma_received_count SET down = down - OLD.action
        WHERE name = (
            SELECT cleaned_nick FROM nick_metadata
                INNER JOIN reacji_message ON nick_metadata.id = reacji_message.nick_id
                WHERE reacji_message.id = OLD.reacji_message_id
...
CREATE TRIGGER delete_increase_sidevote_reacji_karma_count_add AFTER DELETE ON reacji_votes
  WHEN OLD.amount = 0 BEGIN
    ...
    UPDATE karma_received_count SET side = side - OLD.action
        WHERE name = (
            SELECT cleaned_nick FROM nick_metadata
                INNER JOIN reacji_message ON nick_metadata.id = reacji_message.nick_id
                WHERE reacji_message.id = OLD.reacji_message_id
