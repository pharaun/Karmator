...
--
runServer
    :: (ServerState a b -> IO ServerEvent)
    -> (ServerState a b -> IO ())
    -> ServerConfig a
    -> TQueue (BotEvent b, TQueue (BotCommand b))
    -> IO ()
runServer establish lose sc queue = PNT.withSocketsDo $
...
--
messagePump :: (Monad m, MonadIO m) => ServerState a b -> Consumer (BotEvent b) m ()
...
--
messageVacuum :: (Monad m, MonadIO m) => ServerState a b -> Producer (BotCommand b) m ()
...
--
emitConnectionEstablished :: ServerState a b -> IO ()
...
-- TODO: merge with ircConfig this does nothing unique with the config
--
emitConnectionLoss :: ServerState a b -> IO ()
