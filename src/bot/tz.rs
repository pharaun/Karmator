                    ...
                    Some(user_tz) => {
                        match user_tz.tz.parse() {
                            Err(_) => {
                                event
                                    .send_reply(
                                        "Internal error, unable to parse your slack timezone",
                                    )
                                    .await;
                            }
                            Ok(tz) => {
                                ...
                                match convert_naive_time(nt, tz) {
                                    None => {
                                        event
                                            .send_reply("Internal error, timezone happened!")
                                            .await;
                                    }
...
fn tz_abbv(input: &str) -> IResult<&str, (Tz, Option<&str>)> {
    alt((
        map(tag_no_case("SFO"), |_| {
            (chrono_tz::America::Los_Angeles, None)
        }),
        ...
        map(tag_no_case("PST"), |_| {
            (chrono_tz::America::Los_Angeles, Some("SFO"))
        }),
        map(tag_no_case("PDT"), |_| {
            (chrono_tz::America::Los_Angeles, Some("SFO"))
        }),
        map(tag_no_case("EST"), |_| {
            (chrono_tz::America::Toronto, Some("TOR"))
        }),
        map(tag_no_case("EDT"), |_| {
            (chrono_tz::America::Toronto, Some("TOR"))
        }),
        map(tag_no_case("BST"), |_| {
            (chrono_tz::Europe::London, Some("LON"))
        }),
        map(tag_no_case("GMT"), |_| {
            (chrono_tz::Europe::London, Some("LON"))
        }),
        map(tag_no_case("CEST"), |_| {
...
mod test_parser {
    ...
    #[test]
    fn test_meridiem() {
        assert_eq!(meridiem("AM"), Ok(("", "AM")));

        assert_eq!(meridiem("pm"), Ok(("", "PM")));
    }

    #[test]
    fn test_time() {
        assert_eq!(time("0200"), Ok(("", "0200")));
        assert_eq!(time("1700"), Ok(("", "1700")));
        assert_eq!(time("2"), Ok(("", "2")));
        assert_eq!(time("12"), Ok(("", "12")));
        assert_eq!(time("3:03"), Ok(("", "3:03")));
        assert_eq!(time("12:11"), Ok(("", "12:11")));
    }
    ...
    fn test_convert_naive_time() {
        ...
        assert_eq!(conv_req.time(), NaiveTime::from_hms_opt(17, 0, 0).unwrap(),);
        assert_eq!(conv_req.timezone(), chrono_tz::America::Los_Angeles,);
    }
    ...
    fn test_convert_to_locales() {
        ...
        assert_eq!(out_req.pacific.timezone(), chrono_tz::America::Los_Angeles,);
    }
}
